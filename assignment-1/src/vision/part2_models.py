...
class HybridImageModel(nn.Module):
    ...
    def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
        ...
        single_channel = create_Gaussian_kernel_2D(cutoff_frequency)
        kernel = single_channel[np.newaxis, np.newaxis, :, :] * np.ones((self.n_channels, 1, 1, 1))
    ...
    def low_pass(self, x: torch.Tensor, kernel: torch.Tensor):
        ...
        filtered_image = nn.functional.conv2d(
          input = x,
          weight = kernel,
    ...
    def forward(self, image1: torch.Tensor, image2: torch.Tensor, cutoff_frequency: torch.Tensor):
        """
        ...
        high_frequencies = torch.subtract(image2, self.low_pass(image2, kernel))
